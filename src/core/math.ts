...
export class MonteCarloSimulator {
  /**
   * Run Monte Carlo simulation to estimate probability with confidence interval
   * @param lambdaHome - Home team expected goals
   * @param muAway - Away team expected goals
   * @param threshold - Goal threshold
   * @param isUnder - True for Under market, false for Over
  ...
  static run(
    ...
    const matrix = DixonColes.calculateScoreMatrix(lambdaHome, muAway, rho);
    const outcomes: { hit: boolean; prob: number }[] = [];
    ...
    const confidenceInterval95 = 1.96 * Math.sqrt((mean * (1 - mean)) / iterations);

    return {
      mean,
      median: mean,
      confidenceInterval: [mean - confidenceInterval95, mean + confidenceInterval95]
    };
  }
}
