...
export interface RefereeProfile { name:string; avgCardsPerGame:number; avgPenaltiesPerGame:number; homeWinRate:number; tendency:'STRICT'|'LENIENT'|'AVERAGE'; gamesOfficiated:number; strictness?: number; }
...
export interface MatchContext { referee?:RefereeProfile; homeStyle?:TeamStyleProfile; awayStyle?:TeamStyleProfile; league?:string; homeSeasonXG?:number; awaySeasonXG?:number; homeSeasonXGA?:number; awaySeasonXGA?:number; homeTier?: number; awayTier?: number; isDerby?: boolean; sampleSize?: number; date?:string; marketOdds?:{ pinnacleOver15?:number; pinnacleUnder15?:number; pinnacleUnder35?:number; pinnacleOver35?:number; }; groundingLog?:{ citations:Citation[]; varianceAlerts:string[]; }; audit?:{ signalIntegrity:string; alphaAdjustment:string; redCardRegime:string; dataReliability:string; sampleSize:number; }; }
