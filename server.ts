import express from "express"; import path from "path"; import { createServer as createViteServer } from "vite"; import { performAnalysis } from "./src/services/geminiService"; import { BacktestService } from "./src/services/backtestService"; import { DataService } from "./src/services/dataService";
async function startServer() {
  ...
  app.post("/api/analyze", async (req, res) => { try { res.json(await performAnalysis(req.body)); } catch (e: any) { res.status(500).json({ error: e.message }); } });
